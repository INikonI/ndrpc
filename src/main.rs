...
fn main() {
    ...
    let logical_cores: usize = sysinfo_system.processors().len();
    let cpu_brand: String = sysinfo_system.global_processor_info().brand().to_owned();
    ...
    loop {
        ...
        let current_freq: f64 = {
            let result: Vec<HashMap<String, Variant>> = wmi_con.raw_query("SELECT PercentProcessorPerformance FROM Win32_PerfFormattedData_Counters_ProcessorInformation WHERE Name LIKE \"_Total\"").unwrap();
...
fn main() {
    ...
    // precompute some info
    let physical_cores: usize = sysinfo_system.physical_core_count().expect("Failed to get physical core count");
    let logical_cores: usize = sysinfo_system.cpus().len();
    let cpu_brand: String = sysinfo_system.global_cpu_info().brand().to_owned();
    let total_memory: f64 = (sysinfo_system.total_memory() as f64 / 1024_f64 / 1024_f64).round();
